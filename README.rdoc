= Slave Engine
  ...
  - noun
  1. The Inhumans' holiest relic and birthright, built from Kree technology, unearthed by The Unspoken to take revenge on the humans and turn them to Alpha Primitives.
  2. A Rails Engine which contains User and Role management, including rudimentary session management (login/logout).  Comes bundled with a default Admin role and a Users admin panel, only accessible to users with the Admin role.
